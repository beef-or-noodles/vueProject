  ...
  export default {
    data() {
      return {
        ...
        divWidth: '', //盒子平均宽度
        oldLength: 0,//原始长度
        interval: this.intervalN, //偏移间隔
        index: this.colNum,//显示几列
      }
    },
    mounted() {
      var data = this.img
    ...
    watch: {
      img: function (val) { //添加数据
        this.oldLength = this.imgData.length;
        this.imgData.push(...val);
      }
    },
    updated() {
      this.initImg();
    ...
    methods: {
      ...
      initImg() {
        ...
        _this.arrHeight = new Array(index).fill(0) //每列的高度
        _this.arrLeft = new Array(index).fill(0) //每列偏移量
        //计算该有的偏移量
        ...
        for (var i = 0; i < _this.imgData.length; i++) {
          imgOload[i].onload = (function () {
            ...
            _this.arrHeight[k] = _this.arrHeight[k] + div[i].offsetHeight + interval; //更新列高度
